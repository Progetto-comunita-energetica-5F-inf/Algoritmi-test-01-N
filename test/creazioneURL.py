...
def inserisci_dati_in_database(json_data):
    #try:
        ...
        for dato in dati_stimati:
            ...
            print(Datatime)
            Data = Datatime.replace("T"," ").replace(".0000000Z","")
            print(Data)

            cursor.execute("INSERT INTO Meteo ( Datatime,Pressione, Temperatura, Velocità_vento, Direzione_vento, Umidità, Morfologia, Precipitazioni, Irraggiamento) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", (Data,0,air_temp,0,0,0,0,0, ghi ))
...
first_line = obj["estimated_actuals"][0]
#for key in obj:
#  print(key," : ", obj[key]);


first_query_data = json.dumps(first_line)  # Get the first 10 data points

#inserisci_dati_in_database(first_query_data)    